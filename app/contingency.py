...
def sincronizar_cola():
    """Se ejecuta al recuperar la red: vuelca la cola local a la BD central."""
    ...
    for evento in cola:
        conn.execute(
            "INSERT INTO registros (placa, tipo, metodo, timestamp) VALUES (?, ?, ?, ?)",
            (evento["placa"], evento["tipo"], evento["metodo"], evento["timestamp"]),
        )
    conn.commit()
    conn.close()
    n = len(cola)
